...
const INTEREST_CHART = RATE_BUCKETS.map(([_, size]) =>
	Math.max(
		0.1,
		dn.toNumber(size) /
			Math.max(...RATE_BUCKETS.map(([_, size]) => dn.toNumber(size))),
	),
);
...
export default function InputFieldFixture() {
	...
	const ebusdRedeemableInFront = dn.format(getDebtBeforeIndex(valueIndex), {
		compact: true,
	});

	const ebusdInterestPerYear = parsedValue
		? dn.format(dn.mul(DEBT, dn.div(parsedValue, 100)), {
				digits: 2,
				trailingZeros: true,
			})
		: "−";
				...
				contextual={
					...
					>
						<Slider
							...
							onChange={(value) => {
								setValue(
									String(Math.round(lerp(RATE_MIN, RATE_MAX, value) * 10) / 10),
								);
							}}
							value={norm(
								parsedValue ? dn.toNumber(parsedValue) : 0,
								RATE_MIN,
								RATE_MAX,
							)}
						/>
