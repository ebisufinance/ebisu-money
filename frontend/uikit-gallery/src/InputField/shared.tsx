...
export function InputFieldFixture({
		...
		.with("deposit", () => (
			<Dropdown
				...
				items={[
					...
					itemRow("WEETH", "weETH", "50.00"),
					itemRow("EZETH", "ezETH", "60.00"),
				]}
			/>
		))
	...
	const secondaryStart = match(fixture)
		.with(
			"deposit",
			() =>
				`${parsedValue ? dn.format(dn.mul(parsedValue, ETH_PRICE_USD), 2) : "−"}  USD`,
		)
		.with(
			"borrow",
	...
	const secondaryEnd = match(fixture)
		.with("deposit", () => (
			<TextButton label="Max 10.00 ETH" onClick={() => setValue("10")} />
		))
		.with("borrow", () => (
			<div
				style={{
...
function IconEth() {
	return (
		<svg fill="none" height="24" width="24" xmlns="http://www.w3.org/2000/svg">
			...
			<path
				fill="#000"
				d="m11.998 10.031-4.639 2.11L12 14.881l4.638-2.742z"
				opacity=".6"
			/>
