    ...
    describe("Stability Pool Withdrawal", async () => {
      ...
      it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
          whale,
          whale,
          0,
          {
        ...
        // Whale transfers 10k Ebusd to A, B and C who then deposit it to the SP
        const depositors = [alice, bob, carol];
        for (const account of depositors) {
          await ebusdToken.transfer(account, dec(10000, 18), { from: whale });
          await th.provideToSPAndClaim(contracts, dec(10000, 18), {
            from: account,
          });
        }

        // Defaulter opens trove with 200% ICR and 10k Ebusd net debt
        ...
        // Check depositors' compounded deposit is 6666.66 Ebusd and ETH Gain is 33.16 ETH
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: alice,
        });
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: bob,
        });
        const txC = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: carol,
        });
      ...
      it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
        ...
        // Check depositors' compounded deposit is 3333.33 Ebusd and ETH Gain is 66.33 ETH
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: alice,
        });
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
      ...
      it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
        ...
        // Check depositors' compounded deposit is 0 Ebusd and ETH Gain is 99.5 ETH
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: alice,
        });
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
        ...
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(alice)).toString(), "0"),
          10000
        );
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(bob)).toString(), "0"),
          10000
        );
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(carol)).toString(), "0"),
          10000
        );
      ...
      it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing Ebusd", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
        ...
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(5000, 18)),
          defaulter_1,
          defaulter_1,
          0,
        ...
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(7000, 18)),
          defaulter_2,
          defaulter_2,
          0,
      ...
      it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing Ebusd", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
      ...
      it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
        ...
        // Whale transfers 10k, 20k, 30k Ebusd to A, B and C respectively who then deposit it to the SP
        await ebusdToken.transfer(alice, dec(10000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(10000, 18), {
          from: alice,
        });
        await ebusdToken.transfer(bob, dec(20000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: bob });
        await ebusdToken.transfer(carol, dec(30000, 18), { from: whale });
        ...
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(20000, 18), {
          from: bob,
        });
        const txC = await th.withdrawFromSPAndClaim(contracts, dec(30000, 18), {
          from: carol,
        });
      ...
      it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
        ...
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(1000000, 18)),
          whale,
          whale,
          0,
        ...
        /* Depositors provide:-
         ...
         Bob:  456000 Ebusd
         Carol: 13100 Ebusd */
        // Whale transfers Ebusd to  A, B and C respectively who then deposit it to the SP
        await ebusdToken.transfer(alice, dec(2000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(2000, 18), { from: alice });
        await ebusdToken.transfer(bob, dec(456000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(456000, 18), { from: bob });
        await ebusdToken.transfer(carol, dec(13100, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(13100, 18), {
          from: carol,
        });

        /* Defaulters open troves

         Defaulter 1: 207000 Ebusd & 2160 ETH
         Defaulter 2: 5000 Ebusd & 50 ETH
         Defaulter 3: 46700 Ebusd & 500 ETH
      */
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("207000000000000000000000"),
          defaulter_1,
          defaulter_1,
          0,
        ...
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(5, 21)),
          defaulter_2,
          defaulter_2,
          0,
        ...
        const defaulter_3_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("46700000000000000000000"),
          defaulter_3,
          defaulter_3,
          0,
        ...
        // Depositors attempt to withdraw everything
        const txA = await th.withdrawFromSPAndClaim(
          contracts,
          dec(500000, 18),
          { from: alice }
        );
        const txB = await th.withdrawFromSPAndClaim(
          contracts,
          dec(500000, 18),
          { from: bob }
        );
        const txC = await th.withdrawFromSPAndClaim(
          contracts,
          dec(500000, 18),
          { from: carol }
        );
        ...
        assert.isAtMost(
          th.getDifference(alice_ETHWithdrawn, "11486945446800000000"),
          10000000000
        );
        assert.isAtMost(
          th.getDifference(bob_ETHWithdrawn, "2619023561880000000000"),
          10000000000
        );
        assert.isAtMost(
          th.getDifference(carol_ETHWithdrawn, "75239492676700000000"),
          10000000000
        );
      });
      ...
      it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 Ebusd.  A, B, C, D withdraw correct Ebusd deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
        ...
        await ebusdToken.transfer(dennis, dec(10000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(10000, 18), {
          from: dennis,
        });
        ...
        const txD = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: dennis,
        });
      ...
      it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 Ebusd.  A, B, C, D withdraw correct Ebusd deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
      ...
      it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct Ebusd deposit and ETH Gain", async () => {
        // Whale opens Trove with 1m ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(1000000, 18)),
      ...
      */
        // Whale transfers Ebusd to  A, B and C respectively who then deposit it to the SP
        await ebusdToken.transfer(alice, dec(60000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(60000, 18), {
          from: alice,
        });
        await ebusdToken.transfer(bob, dec(20000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: bob });
        await ebusdToken.transfer(carol, dec(15000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(15000, 18), {
          from: carol,
        });

        /* Defaulters open troves:
         Defaulter 1:  10000 Ebusd, 100 ETH
         Defaulter 2:  25000 Ebusd, 250 ETH
         Defaulter 3:  5000 Ebusd, 50 ETH
         Defaulter 4:  40000 Ebusd, 400 ETH
      */
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(10000, 18)),
        ...
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(25000, 18)),
          defaulter_2,
          defaulter_2,
          0,
        ...
        const defaulter_4_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(40000, 18)),
          defaulter_4,
          defaulter_4,
          0,
        ...
        // Dennis provides 25000 Ebusd
        await ebusdToken.transfer(dennis, dec(25000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(25000, 18), {
          from: dennis,
        });
        ...
        const txD = await th.withdrawFromSPAndClaim(
          contracts,
          dec(100000, 18),
          { from: dennis }
        );
      ...
      it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 Ebusd.  A, B, C, D withdraw correct Ebusd deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
      ...
      it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct Ebusd deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
        ...
        /* Initial deposits:
         ...
         Dennis: 40000 Ebusd
      */
        // Whale transfers Ebusd to  A, B,C and D respectively who then deposit it to the SP
        await ebusdToken.transfer(alice, dec(20000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(20000, 18), {
          from: alice,
        });
        await ebusdToken.transfer(bob, dec(25000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(25000, 18), { from: bob });
        await ebusdToken.transfer(carol, dec(12500, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(12500, 18), {
          from: carol,
        });
        await ebusdToken.transfer(dennis, dec(40000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(40000, 18), {
          from: dennis,
        });

        /* Defaulters open troves:
         Defaulter 1: 10000 Ebusd
         Defaulter 2: 20000 Ebusd
         Defaulter 3: 30000 Ebusd
         Defaulter 4: 5000 Ebusd
      */
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(10000, 18)),
        ...
        // Increasing the price for a moment to avoid pending liquidations to block withdrawal
        await priceFeed.setPrice(dec(200, 18));
        const txD = await th.withdrawFromSPAndClaim(contracts, dec(40000, 18), {
          from: dennis,
        });
        await priceFeed.setPrice(dec(100, 18));
      ...
      it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 Ebusd. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct Ebusd deposit and ETH Gain", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
        ...
        // Whale transfers 10k Ebusd to A, B and D who then deposit it to the SP
        ...
        // Carol makes deposit
        await ebusdToken.transfer(carol, dec(10000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(10000, 18), {
          from: carol,
        });
      ...
      // A, B withdraw 0Ebusd & 100e
      // C, D withdraw 5000Ebusd  & 500e
      it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
        ...
        // Whale transfers 10k Ebusd to A, B who then deposit it to the SP
        ...
        // Defaulter 1 liquidated. 20000 Ebusd fully offset with pool.
        await troveManager.liquidate(defaulter_1_TroveId, { from: owner });

        // Carol, Dennis each deposit 10000 Ebusd
        ...
        // Defaulter 2 liquidated. 10000 Ebusd offset
        await troveManager.liquidate(defaulter_2_TroveId, { from: owner });
        ...
        // Expect Alice And Bob's compounded deposit to be 0 Ebusd
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(alice)).toString(), "0"),
          10000
        );
        ...
        // Expect Carol And Dennis' compounded deposit to be 50 Ebusd
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(carol)).toString(),
            "5000000000000000000000"
      ...
      it("withdrawFromSP(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
        ...
        // Defaulter 1 liquidated. 10--0 Ebusd fully offset, Pool remains non-zero
        await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
        ...
        // Defaulter 2 liquidated. 1--00 Ebusd, empties pool
        await troveManager.liquidate(defaulter_2_TroveId, { from: owner });
        ...
        // Defaulter 3 liquidated. 10000 Ebusd fully offset, Pool remains non-zero
        await troveManager.liquidate(defaulter_3_TroveId, { from: owner });
        ...
        // Defaulter 4 liquidated. 10000 Ebusd, empties pool
        await troveManager.liquidate(defaulter_4_TroveId, { from: owner });
      ...
      // A, B withdraw 0 Ebusd & 100e
      // C, D withdraw 5000 Ebusd  & 50e
      it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
        ...
        // Carol, Dennis, Erin each deposit 10000, 20000, 30000 Ebusd respectively
        await ebusdToken.transfer(carol, dec(10000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(10000, 18), {
          from: carol,
        });

        await ebusdToken.transfer(dennis, dec(20000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(20000, 18), {
          from: dennis,
        });

        await ebusdToken.transfer(erin, dec(30000, 18), { from: whale });
        ...
        const txD = await th.withdrawFromSPAndClaim(contracts, dec(20000, 18), {
          from: dennis,
        });
        const txE = await th.withdrawFromSPAndClaim(contracts, dec(30000, 18), {
          from: erin,
        });
      ...
      // A deposits 10000
      // L1, L2, L3 liquidated with 10000 Ebusd each
      // A withdraws all
      ...
      it("withdrawFromSP(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation", async () => {
        ...
        // Defaulter 1,2,3 withdraw 10000 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(10000, 18)),
          defaulter_1,
      ...
      // A,B deposit 10000 Ebusd
      // L1 cancels 20000 Ebusd, 2E
      // B,C deposits 10000 Ebusd
      // L2 cancels 20000 Ebusd, 2E
      // E,F deposit 10000 Ebusd
      // L3 cancels 20000, 200E
      // G,H deposits 10000
      // L4 cancels 20000, 200E

      // Expect all depositors withdraw 0 Ebusd and 100 ETH
      ...
      it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
        ...
        // Alice, Bob each deposit 10k Ebusd
        ...
        // Defaulter 1 liquidated. 20k Ebusd fully offset with pool.
        await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
        ...
        // Erin, Flyn each deposit 10000 Ebusd
        ...
        // Defaulter 3 liquidated. 10000 Ebusd offset
        await troveManager.liquidate(defaulter_3_TroveId, { from: owner });

        // Graham, Harriet each deposit 10000 Ebusd
        ...
        // Defaulter 4 liquidated. 10k Ebusd offset
        await troveManager.liquidate(defaulter_4_TroveId, { from: owner });
        ...
        const txE = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: erin,
        });
        const txF = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: flyn,
        });
        const txG = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: graham,
        });
        const txH = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), {
          from: harriet,
        });
        ...
        // Expect all deposits to be 0 Ebusd
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(alice)).toString(), "0"),
          100000
        );
        ...
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(erin)).toString(), "0"),
          100000
        );
        assert.isAtMost(
          th.getDifference((await ebusdToken.balanceOf(flyn)).toString(), "0"),
          100000
        );
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(graham)).toString(),
            "0"
          ),
          100000
        );
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(harriet)).toString(),
            "0"
          ),
          100000
        );
      ...
      // A withdraws all
      // B deposits 10000
      // L2 of 9900 Ebusd, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
      ...
      // in helper functon getOpenTroveEbusdAmount due to now-zero borrow fees. Double-check this when we write new SP arithmetic tests and fix the "P" issue.
      it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        // Defaulter 1 withdraws 'almost' 10000 Ebusd:  9999.99991 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("9999999910000000000000"),
          defaulter_1,
          defaulter_1,
          0,
          { from: defaulter_1, value: dec(100, "ether") }
        ...
        // Defaulter 2 withdraws 9900 Ebusd
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(9900, 18)),
          defaulter_2,
          defaulter_2,
          0,
        ...
        th.assertIsApproximatelyEqual(
          (await stabilityPool.P()).toString(),
          dec(9, 9),
          1
        );
        ...
        // Defaulter 2 liquidated.  9900 Ebusd liquidated. P altered by a factor of 1-(9900/10000) = 0.01.  Scale changed.
        await troveManager.liquidate(defaulter_2_TroveId, { from: owner });
        ...
        // Expect Bob to withdraw 1% of initial deposit (100 Ebusd) and all the liquidated ETH (60 ether)
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(bob)).toString(),
      ...
      // A deposits 10000
      // L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 Ebusd
      // A withdraws all
      ...
      it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        // Defaulter 1 withdraws 'almost' 10k Ebusd.
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("9999999910000000000000"),
          defaulter_1,
        ...
        // Defaulter 2 withdraws 59400 Ebusd
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("59400000000000000000000"),
          defaulter_2,
          defaulter_2,
          0,
        ...
        await ebusdToken.transfer(carol, dec(20000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(20000, 18), {
          from: carol,
        });

        await ebusdToken.transfer(dennis, dec(30000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(30000, 18), {
          from: dennis,
        });

        // 54000 Ebusd liquidated.  P altered by a factor of 1-(59400/60000) = 0.01. Scale changed.
        const txL2 = await troveManager.liquidate(defaulter_2_TroveId, {
          from: owner,
        });
        ...
        const txC = await th.withdrawFromSPAndClaim(contracts, dec(20000, 18), {
          from: carol,
        });
        const txD = await th.withdrawFromSPAndClaim(contracts, dec(30000, 18), {
          from: dennis,
        });
        ...
        /* Expect depositors to withdraw 1% of their initial deposit, and an ETH gain
         ...
         Bob:  1000 Ebusd, 55 Ether
         Carol:  2000 Ebusd, 110 Ether
         Dennis:  3000 Ebusd, 165 Ether

         Total: 6000 Ebusd, 300 Ether
      */
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(bob)).toString(),
      ...
      // A make deposit 10000 Ebusd
      // L1 brings P to 1e-5*P. L1:  9999.9000000000000000 Ebusd
      // A withdraws
      // B makes deposit 10000 Ebusd
      // L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 Ebusd total deposits)
      // B withdraws
      ...
      it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        // Defaulter 1 and default 2 each withdraw 9999.999999999 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(99999, 17)),
          defaulter_1,
          defaulter_1,
          0,
          { from: defaulter_1, value: dec(100, "ether") }
        ...
        const txL1 = await troveManager.liquidate(defaulter_1_TroveId, {
          from: owner,
        });
        assert.isTrue(txL1.receipt.status);
        th.logBN("P", await stabilityPool.P()); //  0.000009999999999999, so 1 wei less than expected
        th.assertIsApproximatelyEqual(
          await stabilityPool.P(),
          toBN(dec(1, 13))
        ); // P decreases. Expect P = 1e(18-5) = 1e13
        assert.equal(await stabilityPool.currentScale(), "0");
        ...
        // Bob should withdraw 1e-5 of initial deposit: 0.1 Ebusd and the full ETH gain of 100 ether
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(bob)).toString(),
            dec(1, 17)
      ...
      // L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 Ebusd total deposits)
      // B withdraws
      // expect d(B) = d0(B) * 1e-5
      // expect B gets entire ETH gain from L2
      //
      ...
      it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
        ...
        // Defaulter 1 and default 2 withdraw up to debt of 9999.9 Ebusd and 59999.4 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("9999900000000000000000"),
          defaulter_1,
          defaulter_1,
          0,
          { from: defaulter_1, value: dec(100, "ether") }
        ...
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("59999400000000000000000"),
          defaulter_2,
          defaulter_2,
          0,
        ...
        // Increasing the price for a moment to avoid pending liquidations to block withdrawal
        await priceFeed.setPrice(dec(200, 18));
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(100, 18), {
          from: alice,
        });
        await priceFeed.setPrice(dec(100, 18));

        // B, C, D deposit 10000, 20000, 30000 Ebusd
        await ebusdToken.transfer(bob, dec(10000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: bob });
        ...
        // {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} Ebusd
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(bob)).toString(),
            dec(1, 17)
        ...
        assert.isAtMost(
          th.getDifference(bob_ETHWithdrawn, dec(9966666667, 10)),
          10000000000
        );
        assert.isAtMost(
          th.getDifference(carol_ETHWithdrawn, dec(19933333333, 10)),
          100000000000
        );
        assert.isAtMost(
          th.getDifference(dennis_ETHWithdrawn, dec(299, 18)),
          100000000000
        );
      });

      // A make deposit 10000 Ebusd
      // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 Ebusd
      ...
      it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
        ...
        // Defaulters 1 withdraws 9999.9999999 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("9999999999900000000000"),
          defaulter_1,
          defaulter_1,
          0,
          { from: defaulter_1, value: dec(100, "ether") }
      ...
      /* A make deposit 10000 Ebusd
       L1 brings P to 0.0001P. L1:  9999.900000000000000000 Ebusd, 1 ETH
       B makes deposit 9999.9, brings SP to 10k
       L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 Ebusd, 1 ETH
       C makes deposit 9999.9, brings SP to 10k
       L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 Ebusd, 1 ETH
       D makes deposit 9999.9, brings SP to 10k
       L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 Ebusd, 1 ETH
       expect A, B, C, D each withdraw ~100 Ether
    */
      // TODO: Changes since v1 have introduced very slight precision error in this test. Potentially due to slightly different rounding
      // in helper functon getOpenTroveEbusdAmount due to now-zero borrow fees. Double-check this when we write new SP arithmetic tests and fix the "P" issue.
      it("withdrawFromSP(): Several deposits of 10000 Ebusd span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
        // Whale opens Trove with 100k ETH
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(100000, 18)),
        ...
        // Defaulters 1-4 each withdraw 9999.9 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount("9999900000000000000000"),
          defaulter_1,
        ...
        th.assertIsApproximatelyEqual(
          await stabilityPool.P(),
          toBN(dec(1, 13))
        ); // Expect P decreases to 1e(18-5) = 1e13
        assert.equal(await stabilityPool.currentScale(), "0");

        // B deposits 9999.9 Ebusd
        await ebusdToken.transfer(bob, dec(99999, 17), { from: whale });
        ...
        // C deposits 9999.9 Ebusd
        await ebusdToken.transfer(carol, dec(99999, 17), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(99999, 17), {
          from: carol,
        });

        // Defaulter 3 liquidated
        const txL3 = await troveManager.liquidate(defaulter_3_TroveId, {
          from: owner,
        });
        ...
        // D deposits 9999.9 Ebusd
        await ebusdToken.transfer(dennis, dec(99999, 17), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(99999, 17), {
          from: dennis,
        });

        // Defaulter 4 liquidated
        const txL4 = await troveManager.liquidate(defaulter_4_TroveId, {
          from: owner,
        });
        ...
        assert.equal(await ebusdToken.balanceOf(alice), "0");
        assert.equal(await ebusdToken.balanceOf(alice), "0");
        assert.equal(await ebusdToken.balanceOf(alice), "0");
        // D should withdraw around 0.9999 Ebusd, since his deposit of 9999.9 was reduced by a factor of 1e-5
        assert.isAtMost(
          th.getDifference(
            (await ebusdToken.balanceOf(dennis)).toString(),
            dec(99999, 12)
      ...
      it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
        ...
        // Defaulters 1-3 each withdraw 24100, 24300, 24500 Ebusd (inc gas comp)
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(24100, 18)),
          defaulter_1,
          defaulter_1,
          0,
          { from: defaulter_1, value: dec(200, "ether") }
        ...
        const defaulter_2_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(24300, 18)),
          defaulter_2,
          defaulter_2,
          0,
          { from: defaulter_2, value: dec(200, "ether") }
        ...
        const defaulter_3_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(24500, 18)),
          defaulter_3,
          defaulter_3,
          0,
          { from: defaulter_3, value: dec(200, "ether") }
        ...
        // A, B provide 10k Ebusd
        await ebusdToken.transfer(A, dec(10000, 18), { from: whale });
        await ebusdToken.transfer(B, dec(10000, 18), { from: whale });
        ...
        const A_deposit = await stabilityPool.getCompoundedEbusdDeposit(A);
        const B_deposit = await stabilityPool.getCompoundedEbusdDeposit(B);
        ...
        const EbusdinSP_1 = await stabilityPool.getTotalEbusdDeposits();
        // console.log(`EbusdinSP_1: ${EbusdinSP_1}`)
        assert.equal(EbusdinSP_1, "0");

        // Check SP Ebusd balance is zero
        const SPEbusdBalance_1 = await ebusdToken.balanceOf(
          stabilityPool.address
        );
        // console.log(`SPEbusdBalance_1: ${SPEbusdBalance_1}`)
        assert.equal(SPEbusdBalance_1, "0");

        // Attempt withdrawals
        // Increasing the price for a moment to avoid pending liquidations to block withdrawal
        await priceFeed.setPrice(dec(200, 18));
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(1000, 18), {
          from: A,
        });
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(1000, 18), {
          from: B,
        });
        await priceFeed.setPrice(dec(100, 18));
        ...
        // C, D provide 10k Ebusd
        await ebusdToken.transfer(C, dec(10000, 18), { from: whale });
        await ebusdToken.transfer(D, dec(10000, 18), { from: whale });
        ...
        // Check compounded deposits
        const C_deposit = await stabilityPool.getCompoundedEbusdDeposit(C);
        const D_deposit = await stabilityPool.getCompoundedEbusdDeposit(D);
        ...
        // Check SP tracker is zero
        const EbusdinSP_2 = await stabilityPool.getTotalEbusdDeposits();
        // console.log(`EbusdinSP_2: ${EbusdinSP_2}`)
        assert.equal(EbusdinSP_2, "0");

        // Check SP Ebusd balance is zero
        const SPEbusdBalance_2 = await ebusdToken.balanceOf(
          stabilityPool.address
        );
        // console.log(`SPEbusdBalance_2: ${SPEbusdBalance_2}`)
        assert.equal(SPEbusdBalance_2, "0");

        // Attempt withdrawals
        // Increasing the price for a moment to avoid pending liquidations to block withdrawal
        await priceFeed.setPrice(dec(200, 18));
        const txC = await th.withdrawFromSPAndClaim(contracts, dec(1000, 18), {
          from: C,
        });
        const txD = await th.withdrawFromSPAndClaim(contracts, dec(1000, 18), {
          from: D,
        });
        await priceFeed.setPrice(dec(100, 18));
        ...
        // E, F provide 10k Ebusd
        await ebusdToken.transfer(E, dec(10000, 18), { from: whale });
        await ebusdToken.transfer(F, dec(10000, 18), { from: whale });
        ...
        // Check compounded deposits
        const E_deposit = await stabilityPool.getCompoundedEbusdDeposit(E);
        const F_deposit = await stabilityPool.getCompoundedEbusdDeposit(F);
        ...
        // Check SP tracker is zero
        const EbusdinSP_3 = await stabilityPool.getTotalEbusdDeposits();
        assert.equal(EbusdinSP_3, "0");

        // Check SP Ebusd balance is zero
        const SPEbusdBalance_3 = await ebusdToken.balanceOf(
          stabilityPool.address
        );
        // console.log(`SPEbusdBalance_3: ${SPEbusdBalance_3}`)
        assert.equal(SPEbusdBalance_3, "0");

        // Attempt withdrawals
        const txE = await th.withdrawFromSPAndClaim(contracts, dec(1000, 18), {
          from: E,
        });
        const txF = await th.withdrawFromSPAndClaim(contracts, dec(1000, 18), {
          from: F,
        });
      ...
      it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
        ...
        // Defaulters 1-5 each withdraw up to debt of 9999.9999999 Ebusd
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(99999, 17)),
          defaulter_1,
        ...
        // E deposits 9999.9 Ebusd
        await ebusdToken.transfer(erin, dec(99999, 17), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(99999, 17), { from: erin });

        // Defaulter 5 liquidated
        const txL5 = await troveManager.liquidate(defaulter_5_TroveId, {
          from: owner,
        });
      ...
      it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
        ...
        for (const account of depositors) {
          ...
          await th.provideToSPAndClaim(contracts, dec(1, 36), {
            from: account,
          });
        }
        ...
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(1, 36)),
          defaulter_1,
          defaulter_1,
          0,
        ...
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(1, 36), {
          from: alice,
        });
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(1, 36), {
          from: bob,
        });
        ...
        // Check Ebusd balances
        const aliceEbusdBalance = await ebusdToken.balanceOf(alice);
        const aliceExpectedEbusdBalance = web3.utils.toBN(dec(5, 35));
        const aliceEbusdBalDiff = aliceEbusdBalance
          .sub(aliceExpectedEbusdBalance)
          .abs();

        assert.isTrue(aliceEbusdBalDiff.lte(toBN(dec(1, 18)))); // error tolerance of 1e18

        const bobEbusdBalance = await ebusdToken.balanceOf(bob);
        const bobExpectedEbusdBalance = toBN(dec(5, 35));
        const bobEbusdBalDiff = bobEbusdBalance
          .sub(bobExpectedEbusdBalance)
          .abs();

        assert.isTrue(bobEbusdBalDiff.lte(toBN(dec(1, 18))));
      ...
      // TODO: Changes since v1 have made the error margin in this test  i.e. aliceEbusdBalanceDiff increase 100x (but still low relative to the huge values used in test).
      // Potentially due to slightly different rounding in helper getOpenTroveEbusdAmount due to now-zero borrow fees.
      ...
      it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
        ...
        for (const account of depositors) {
          ...
          await th.provideToSPAndClaim(contracts, dec(1, 38), {
            from: account,
          });
        }

        // Defaulter opens trove with 50e-7 ETH and  5000 Ebusd. 200% ICR
        const defaulter_1_TroveId = await th.openTroveWrapper(
          contracts,
          await getOpenTroveEbusdAmount(dec(5000, 18)),
          defaulter_1,
        ...
        const txA = await th.withdrawFromSPAndClaim(contracts, dec(1, 38), {
          from: alice,
        });
        const txB = await th.withdrawFromSPAndClaim(contracts, dec(1, 38), {
          from: bob,
        });
        ...
        const aliceEbusdBalance = await ebusdToken.balanceOf(alice);
        const aliceExpectedEbusdBalance = toBN(
        ...
        // th.logBN("aliceEbusdBalDiff", aliceEbusdBalDiff);
        ...
        assert.isTrue(aliceEbusdBalDiff.lte(toBN(dec(1, 20))));

        const bobEbusdBalance = await ebusdToken.balanceOf(bob);
        const bobExpectedEbusdBalance = toBN(
          "99999999999999997500000000000000000000"
        );
        const bobEbusdBalDiff = bobEbusdBalance
          .sub(bobExpectedEbusdBalance)
        ...
        assert.isTrue(bobEbusdBalDiff.lte(toBN("100000000000000000000")));
