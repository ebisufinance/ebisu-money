...
contract(
  ...
  async (accounts) => {
    ...
    */
      ...
      assert.equal((await ebusdToken.balanceOf(owner)).toString(), "0");
    });
    ...
    */
    ...
    it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
      ...
      const freddy_rawColl = (
        await troveManager.Troves(freddyTroveId)
      )[1].toString();
    ...
    */
    ...
    it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
      ...
      const { troveId: ATroveId, collateral: A_coll } = await openTrove({
        ICR: toBN(dec(200, 16)),
        extraEbusdAmount: dec(100000, 18),
        extraParams: { from: A },
      });
      ...
      const B_entireColl_1 = (
        await th.getEntireCollAndDebtByAddress(contracts, B)
      ).entireColl;
      const C_entireColl_1 = (
        await th.getEntireCollAndDebtByAddress(contracts, C)
      ).entireColl;
      const D_entireColl_1 = (
        await th.getEntireCollAndDebtByAddress(contracts, D)
      ).entireColl;
      const E_entireColl_1 = (
        await th.getEntireCollAndDebtByAddress(contracts, E)
      ).entireColl;
      ...
      const B_entireColl_2 = (
        await th.getEntireCollAndDebtByAddress(contracts, B)
      ).entireColl;
      const D_entireColl_2 = (
        await th.getEntireCollAndDebtByAddress(contracts, D)
      ).entireColl;
      const E_entireColl_2 = (
        await th.getEntireCollAndDebtByAddress(contracts, E)
      ).entireColl;
      ...
      const B_entireColl_3 = (
        await th.getEntireCollAndDebtByAddress(contracts, B)
      ).entireColl;
      const D_entireColl_3 = (
        await th.getEntireCollAndDebtByAddress(contracts, D)
      ).entireColl;
    ...
    it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
      ...
      // Check entireColl for each trove:
      const A_entireColl_0 = (
        await th.getEntireCollAndDebtByAddress(contracts, A)
      ).entireColl;
      const B_entireColl_0 = (
        await th.getEntireCollAndDebtByAddress(contracts, B)
      ).entireColl;
      const C_entireColl_0 = (
        await th.getEntireCollAndDebtByAddress(contracts, C)
      ).entireColl;
      const D_entireColl_0 = (
        await th.getEntireCollAndDebtByAddress(contracts, D)
      ).entireColl;
      const E_entireColl_0 = (
        await th.getEntireCollAndDebtByAddress(contracts, E)
      ).entireColl;
    ...
    it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
      ...
      // Expect Bob now holds all Ether and EbusdDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 Ebusd (10 each for gas compensation)
      const bob_Coll = (await troveManager.Troves(bobTroveId))[1]
        .add(await troveManager.getPendingCollReward(bobTroveId))
        .toString();

      const bob_EbusdDebt = (await troveManager.Troves(bobTroveId))[0]
        .add(await troveManager.getPendingEbusdDebtReward(bobTroveId))
        .toString();
    ...
    it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
      ...
      /* Bob rewards:
       L1: 1/2*0.995 ETH, 55 Ebusd
       L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 EbusdDebt

       coll: 3.1195 ETH
       debt: 233.77 EbusdDebt

       Alice rewards:
       L1 1/2*0.995 ETH, 55 Ebusd
       L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 EbusdDebt

       coll: 1.8705 ETH
       debt: 146.23 EbusdDebt

       totalColl: 4.99 ETH
       totalDebt 380 Ebusd (includes 50 each for gas compensation)
    */
      const bob_Coll = (await troveManager.Troves(bobTroveId))[1]
        .add(await troveManager.getPendingCollReward(bobTroveId))
        .toString();
      ...
      const alice_EbusdDebt = (await troveManager.Troves(aliceTroveId))[0]
        .add(await troveManager.getPendingEbusdDebtReward(aliceTroveId))
        .toString();
      ...
      assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000);
      assert.isAtMost(th.getDifference(bob_EbusdDebt, expected_B_debt), 10000);
      ...
      assert.isAtMost(
        th.getDifference(alice_EbusdDebt, expected_A_debt),
        10000
      );
    ...
    */
      ...
      th.assertIsApproximatelyEqual(
        (await ebusdToken.balanceOf(owner)).toString(),
        "0"
      );
    });
    ...
    */
    ...
    it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
      ...
      // Expect Bob now holds all Ether and EbusdDebt in the system: 2.5 Ether and 300 Ebusd
    ...
    it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
      ...
      const entireSystemDebt = (await activePool.getEbusdDebt()).add(
    ...
    */
    ...
    it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
      ...
      const erin_rawColl = (
        await troveManager.Troves(erinTroveId)
      )[1].toString();
