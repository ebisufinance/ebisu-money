...
contract("TroveManager", async (accounts) => {
  ...
  it("A given trove's stake decline is negligible with adjustments and tiny liquidations", async () => {
    ...
    const ATroveId = await th.openTroveWrapper(
      contracts,
      await getOpenTroveEbusdAmount(dec(1, 31)),
      ZERO_ADDRESS,
      ZERO_ADDRESS,
      0,
    ...
    const BTroveId = await th.openTroveWrapper(
      contracts,
      await getOpenTroveEbusdAmount(dec(2, 30)),
      ZERO_ADDRESS,
      ZERO_ADDRESS,
      0,
    ...
    for (const account of tinyTroves) {
      await contracts.WETH.mint(
        account,
        toBN(eth_amount).add(ETH_GAS_COMPENSATION)
      );
      await th.openTroveWrapper(
        contracts,
        await getOpenTroveEbusdAmount(dec(1, 22)),
        ZERO_ADDRESS,
        ZERO_ADDRESS,
        0,
